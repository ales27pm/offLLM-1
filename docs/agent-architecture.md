...
- `AgentOrchestrator` wires together the language model service, memory, prompt builder, tool handler, and plugin system. The `run` method retrieves long- and short-term context, builds a system prompt from the prompt registry, calls the LLM, parses any `TOOL_CALL` envelopes, executes the referenced tools, and then issues a follow-up LLM call that incorporates tool output before persisting the exchange to memory.【F:src/core/AgentOrchestrator.js†L1-L238】
- `PromptBuilder` makes the available tool roster explicit by enumerating the registered tools (name, description, JSON schema) and injecting them into the prompt registry templates. Prompt formatting flows through the versioned registry in `prompts/registry.json`, with canonical templates stored under `prompts/v1/` and rendered by `PromptRegistry` + `PromptLoader` so runtime and training prompts stay aligned under a single source of truth.【F:src/core/prompt/PromptBuilder.js†L1-L64】【F:src/core/prompt/PromptRegistry.js†L1-L41】【F:src/core/prompt/PromptLoader.js†L1-L34】【F:prompts/registry.json†L1-L20】【F:prompts/v1/runtime_prompt.json†L1-L8】
- `ToolHandler` implements the dynamic routing layer: it parses `TOOL_CALL` envelopes emitted by the LLM, validates arguments against JSON Schemas before execution, enforces capability allowlists, and returns structured results that are appended to the conversational context.【F:src/core/tools/ToolHandler.js†L1-L188】
...
- `VectorIndexer`, `Retriever`, and `HistoryService` handle the respective responsibilities of embedding new content, fetching similarity matches (with sparse-attention re-ranking), and tracking the sliding conversational window; retrievers can emit telemetry traces through the shared `TelemetrySink` when configured.【F:src/core/memory/services/VectorIndexer.js†L1-L33】【F:src/core/memory/services/Retriever.js†L1-L93】【F:src/core/memory/services/HistoryService.js†L1-L17】【F:src/utils/telemetry.js†L1-L156】
- `ContextEngineer` now exposes a deterministic chunking + retrieval helper that can plug in a custom retriever while emitting retrieval traces for downstream prompt assembly or evaluation workflows.【F:src/services/contextEngineer.js†L1-L75】【F:src/retrieval/chunking.js†L1-L57】【F:src/retrieval/retrievalConfig.js†L1-L18】
...
- Telemetry events capture model interactions, tool calls, and retrieval signals with conservative redaction and SHA-256 hashes, and the `TelemetrySink` keeps the runtime contract minimal while still conforming to the canonical telemetry JSON Schema.【F:src/utils/telemetry.js†L1-L156】【F:schemas/telemetry_event.schema.json†L1-L118】
- The LoRA training pipeline now consumes the shared prompt registry, ensuring training prompts match the runtime tool-call schema and instruction text while emitting an export manifest with dataset and model hashes for reproducible training runs.【F:scripts/train_lora.py†L1-L184】【F:prompts/registry.json†L1-L70】【F:prompts/v1/training_prompt.json†L1-L5】【F:scripts/mlops/write_export_manifest.py†L1-L78】
- Evaluation scripts provide deterministic checks for tool-call formatting and refusal behaviour, emitting gated summaries that enforce non-regression across prompt/tool handling and refusal metrics while persisting structured failures for CI artifacts.【F:scripts/eval/offline_eval.py†L1-L170】【F:scripts/eval/metrics.py†L1-L26】
