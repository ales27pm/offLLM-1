...
- `PromptBuilder` makes the available tool roster explicit by enumerating the registered tools (name, description, parameter schema) and stitching both retrieved context and user input into the prompt template that is handed to the model. Prompt formatting now flows through the versioned registry in `prompts/registry.json`, with canonical prompt templates stored under `prompts/v1/` and loaded by `PromptRegistry.ts` so runtime and training prompts stay aligned under a single source of truth.【F:src/core/prompt/PromptBuilder.js†L1-L35】【F:src/core/prompt/promptTemplate.js†L1-L36】【F:src/core/prompt/PromptRegistry.ts†L1-L88】【F:prompts/registry.json†L1-L70】【F:prompts/v1/runtime_prompt.json†L1-L8】
...
- Evaluation scripts provide deterministic checks for prompt regression (prompt output, tool-call diffs, JSON/refusal/citation expectations, and SARIF export), retrieval recall, and export equivalence, with gated summaries enforcing non-regression across prompt/tool/retrieval/latency/memory metrics.【F:scripts/eval/run_prompt_regression.py†L1-L361】【F:scripts/eval/retrieval_eval.py†L1-L54】【F:scripts/eval/export_equivalence.py†L1-L52】【F:scripts/eval/write_eval_summary.py†L1-L88】
