...
def signals_for_file(repo_root: Path, p: Path, limit_bytes: int) -> Optional[FileSignals]:
    try:
        st = p.stat()
    except Exception:
        return None
...
def find_dup_candidates(file_sigs: list[FileSignals], key: str, min_hits: int = 3) -> dict[str, list[str]]:
...
def build_finetune_axes(search: SearchMap) -> list[FineTuneAxis]:
    ...
    axes.append(FineTuneAxis(
        ...
        dataset_sources=(["prompt_registry: versioned templates"] if search.prompts else ["existing prompts in repo"]),
        evals=[
            "prompt-adherence",
            "format-compliance",
        ],
...
def main() -> int:
    ...
    # Write outputs
    (out_dir / "repo_index.json").write_text(json.dumps([asdict(fs) for fs in file_sigs], indent=2), encoding="utf-8")
    (out_dir / "symbiosis_plan.json").write_text(json.dumps(asdict(plan), indent=2), encoding="utf-8")
